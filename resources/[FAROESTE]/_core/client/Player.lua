            ...
            if cAPI.IsPlayerHorseActive() then
                if not isHorseActivationBlocked then
                    if IsPedInjured(playerHorse) then
                        cAPI.notify("error", _('respawn_horse'))
        ...
        while true do
            ...
            if isHorseInWrithe then
                ...
                if IsPedInWrithe(playerHorse) then
                    cAPI.notify("alert", _('revive_horse'))
...
Citizen.CreateThread(
    function()
        while true do
            ...
            if diff > 1 then
                if isWanted then
                    DrawText(_('revive_horse') .. string.format("%.0f", math.max(diff / 1000, 0)) .. _('segundos'), 0.925, 0.96, 0.25, 0.25, false, 255, 255, 255, 145, 1, 7)
                end
            else
                if isWanted then
                    isWanted = false
